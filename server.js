...
const server = http.createServer((req, res) => {
    ...
    if (method == "GET") {
        ...
        } else if (req.url == "/StudentData") {
            fs.readFile("StudentData.html", "utf8", (err, data) => {
                if (err) {
                    res.writeHead(500);
                    res.end("Server Error");
                } else {
                    console.log("sending StudentData.html file");
                    res.end(data);
                }
            });
        } else if (req.url === "/RegistrationForm") {
            fs.readFile("RegistrationForm.html", "utf8", (err, data) => {
                if (err) {
                    res.writeHead(500);
                    res.end("Server Error");
                } else {
                    console.log("sending RegistrationForm.html file");
                    res.end(data);
                }
            });
        } else{
        ...
        if (req.url === "/RegistrationForm") {
            console.log("inside /RegistrationForm route and post request");
            let body = "";
            req.on("data", (chunk) => {
                body += chunk.toString();
                //  console.log(chunk);
